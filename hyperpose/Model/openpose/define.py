...
# from ...Dataset.mscoco_dataset.define import CocoPart,CocoColor


from ...Dataset.userdef_dataset.define import UserdefPart,UserdefColor

from ...Dataset.userdef_dataset.define import UserdefLimb

# from .define import UserdefPart,UserdefLimb,UserdefColor,Userdef_flip_list

def get_userdef_flip_list():
    flip_list=[]
    for part_idx,part in enumerate(UserdefPart):
        # print(part.value)
        LRflip = str(part).split(".", 1)[1]
        if LRflip[0] == 'L':
            LRfliped = 'R' + LRflip[1:]
            flip_list.append(UserdefPart[LRfliped].value)
        elif LRflip[0] == 'R':
            LRfliped = 'L' + LRflip[1:]
            flip_list.append(UserdefPart[LRfliped].value)
        else:
...
Userdef_flip_list = get_userdef_flip_list()

    # for part_idx,part in enumerate(CocoPart):
    #     # print(part.value)
    #     # wing flip
    #     if(part==CocoPart.RWing):
    #         flip_list.append(CocoPart.LWing.value)
    #     elif(part==CocoPart.LWing):
    #         flip_list.append(CocoPart.RWing.value)
    #     # others
    #     else:
    #         flip_list.append(part.value)
    # return flip_list
