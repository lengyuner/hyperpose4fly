...
from .userdef_dataset import Userdef_dataset
...
def get_dataset(config):
    ...
    if(dataset_type==DATA.MSCOCO):
        ...
        if (model_type == MODEL.LightweightOpenpose or model_type == MODEL.MobilenetThinOpenpose or model_type == MODEL.Openpose):
            from .userdef_dataset.define import opps_input_converter as input_kpt_cvter
            from .userdef_dataset.define import opps_output_converter as output_kpt_cvter
        elif (model_type == MODEL.PoseProposal):
            from .userdef_dataset.define import ppn_input_converter as input_kpt_cvter
            from .userdef_dataset.define import ppn_output_converter as output_kpt_cvter
        dataset = Userdef_dataset(config, input_kpt_cvter, output_kpt_cvter)
        dataset.prepare_dataset()
